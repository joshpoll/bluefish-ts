import * as indigo from 'indigo-ts';
import { Variable, mkVar, AffineExpr, mkAffineConstraint, Constraint, plus, mul, mkEqConstraint, mkConstEqConstraint, weak } from 'indigo-ts';
import _ from 'lodash';
import { Gestalt } from "./gestaltIndigo";
import { BBoxTree, getBBoxValues, makeBBoxVars, bboxVars, BBoxValues, MaybeBBoxValues, BBoxTreeValue, BBoxTreeVV, bboxVarExprs, transformBBox, Transform } from './indigoBBoxTransform';
...
const Strength = {
  required: indigo.required,
  strong: indigo.strong,
  medium: indigo.medium,
  weak: indigo.weak,
  weaker: indigo.weaker,
}
...
const addChildrenConstraints = (bboxTree: BBoxTreeVVE, constraints: Constraint[]): void => {
  ...
  constraints.push(mkAffineConstraint(bboxTree.canvas.bboxVars.left, 'eq', 0, Strength.weak));
  constraints.push(mkAffineConstraint(bboxTree.canvas.bboxVars.top, 'eq', 0, Strength.weak));

  // break ties with competing bbox shrinkwraps from above and below. encourages bounding boxes to
  // fit their contents
  constraints.push(mkAffineConstraint(bboxTree.canvas.bboxVars.width, 'eq', 0, Strength.strong));
  constraints.push(mkAffineConstraint(bboxTree.canvas.bboxVars.height, 'eq', 0, Strength.strong));
  ...
  for (const bboxKey of Object.keys(bboxTree.children)) {
    ...
    if (!widthDefined(bboxTree)) {
      constraints.push(mkAffineConstraint(bboxTree.children[bboxKey].bbox.bboxVars.left, 'eq', bboxTree.canvas.bboxVars.left, widthDefined(bboxTree.children[bboxKey]) ? Strength.medium : Strength.medium));
      constraints.push(mkAffineConstraint(bboxTree.children[bboxKey].bbox.bboxVars.right, 'eq', bboxTree.canvas.bboxVars.right, widthDefined(bboxTree.children[bboxKey]) ? Strength.medium : Strength.medium));
    }

    if (!heightDefined(bboxTree)) {
      constraints.push(mkAffineConstraint(bboxTree.children[bboxKey].bbox.bboxVars.top, 'eq', bboxTree.canvas.bboxVars.top, heightDefined(bboxTree.children[bboxKey]) ? Strength.medium : Strength.medium));
      constraints.push(mkAffineConstraint(bboxTree.children[bboxKey].bbox.bboxVars.bottom, 'eq', bboxTree.canvas.bboxVars.bottom, heightDefined(bboxTree.children[bboxKey]) ? Strength.medium : Strength.medium));
    }
...
const makeBBoxTreeWithRef = (encoding: GlyphWithPath, constraints: Constraint[]): BBoxTreeVVEWithRef => {
  if ("$ref" in encoding) {
    ...
    constraints.push(mkConstEqConstraint(transform.translate.x, 0, weak));
    constraints.push(mkConstEqConstraint(transform.translate.y, 0, weak));
...
const composeTransformVE = (t1: Transform<Variable | AffineExpr>, t2: Transform<Variable | AffineExpr>): Transform<Variable | AffineExpr> => ({
  translate: {
    x: indigo.plus(t1.translate.x, t2.translate.x),
    y: indigo.plus(t1.translate.y, t2.translate.y),
  }
})

const inverseTransformVE = (t: Transform<Variable | AffineExpr>): Transform<Variable | AffineExpr> => ({
  translate: {
    x: indigo.mul(-1, t.translate.x),
    y: indigo.mul(-1, t.translate.y),
  }
})
...
const resolveRefs = (rootBboxTreeWithRef: BBoxTreeVVEWithRef, bboxTreeWithRef: BBoxTreeVVEWithRef, path: string[], transform: Transform<Variable | AffineExpr>, constraints: Constraint[]): BBoxTreeVVE => {
  ...
  if ("$ref" in bboxTreeWithRef) {
    ...
    constraints.push(mkConstEqConstraint(bboxTransform.translate.x, 0, weak));
    constraints.push(mkConstEqConstraint(bboxTransform.translate.y, 0, weak));
...
//           console.log("bbox val constraint", (mkAffineConstraint(bboxTree.bbox.bboxVars[key], 'eq', bboxTree.bbox.bboxValues[key])).toString())
...
export const addBBoxConstraintsWithRef = (bboxTree: BBoxTreeVVEWithRef, constraints: Constraint[]): void => {
  if ("$ref" in bboxTree) {
    ...
    // constraints.push(mkAffineConstraint(bboxTree.bbox.bboxVars.width, 'eq', new Expression(bboxTree.canvas.bboxVars.width)));
    // constraints.push(mkAffineConstraint(bboxTree.bbox.bboxVars.height, 'eq', new Expression(bboxTree.canvas.bboxVars.height)));
    // constraints.push(mkAffineConstraint(bboxTree.bbox.bboxVars.centerX, 'eq', new Expression(bboxTree.canvas.bboxVars.centerX, bboxTree.transform.translate.x)));
    // constraints.push(mkAffineConstraint(bboxTree.bbox.bboxVars.centerY, 'eq', new Expression(bboxTree.canvas.bboxVars.centerY, bboxTree.transform.translate.y)));
  }
}
...
export const addTransformConstraints = (bboxTree: BBoxTreeVVE, constraints: Constraint[], name = "$root"): void => {
  ...
  // bbox = transform(canvas)
  const constr = [];
  constr.push(mkAffineConstraint(bboxTree.bbox.bboxVars.width, 'eq', bboxTree.canvas.bboxVars.width));
  constr.push(mkAffineConstraint(bboxTree.bbox.bboxVars.height, 'eq', bboxTree.canvas.bboxVars.height));
  constr.push(mkAffineConstraint(bboxTree.bbox.bboxVars.centerX, 'eq', plus(bboxTree.canvas.bboxVars.centerX, bboxTree.transform.translate.x)));
  constr.push(mkAffineConstraint(bboxTree.bbox.bboxVars.centerY, 'eq', plus(bboxTree.canvas.bboxVars.centerY, bboxTree.transform.translate.y)));
  ...
  if (bboxTree.inheritFrame) {
    ...
    constraints.push(mkAffineConstraint(bboxTree.transform.translate.x, 'eq', 0));
    constraints.push(mkAffineConstraint(bboxTree.transform.translate.y, 'eq', 0));
  }
...
export default (encoding: Glyph): CompiledAST => {
  ...
  constraints.push(mkAffineConstraint(bboxTree.bbox.bboxVars.left, 'eq', 0));
  constraints.push(mkAffineConstraint(bboxTree.bbox.bboxVars.top, 'eq', 0));
